# server/strategies/kaushalcustomWMASMAPut1/logic.py
...
_spec = importlib.util.spec_from_file_location("kaushalcustomWMASMAPut1_order", _order_path)
...
LOG_PATH = os.path.join(os.path.dirname(__file__), "nifty_atm_wma5_sma1_open_offsetm1_put.log")
...
def _compute_indicators(candles_latest_first: List[list]) -> Dict[str, Any]:
    ...
    return {
        ...
        # indicators computed on Heikin-Ashi with offset -1 on both series
...
# =========================================================
# IMPORTANT: must match strategies_routes.py which put:
...
def stream_forever(user_id: int, ctx: Optional[Dict[str, Any]] = None) -> None:
    ...
    _log(f"[NIFTY_ATM_WMA5_SMA1_OPEN_OFFSETM1_PUT] stream_forever started uid={user_id} broker={broker} tf={TIMEFRAME_MIN}m")
    ...
    while True:
        try:
            ...
            if _is_strategy_stopped(user_id):
                _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] Strategy STOPPED/EXITED for user={user_id}. Exiting stream_forever.")
                break
            ...
            if not broker:
                broker = _resolve_broker_for_user(user_id, prefs=prefs) or ""
                if broker:
                    _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] broker resolved dynamically: {broker}")
            ...
            if end_time and now >= end_time:
                _log(
                    f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] End time reached ({end_time.strftime('%Y-%m-%d %H:%M %z')}). "
                    "Auto-exit + stop."
                )
                ...
                if not broker:
                    broker = _resolve_broker_for_user(user_id, prefs=prefs) or ""
                if not broker:
                    _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] broker not resolved; cannot AUTO-EXIT order at end time")
                else:
                    if in_trade and current_option_key and current_option_sym:
                        ...
                        _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] END-TIME EXIT res=", ex)
                    if flatten_today_positions:
                        flatten_today_positions(
                            ...
                            log_prefix="[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] ",
                        )
                _mark_strategy_stopped(user_id, reason="time_end")
                _log("[NIFTY_ATM_WMA5_SMA2_OPEN] Strategy STOPPED after end time. Exiting stream_forever.")
                break
            ...
            if now >= exit_cutoff_dt:
                if in_trade and current_option_key and current_option_sym:
                    try:
                        _log(
                            f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT][EXIT] time_cutoff ts={now.isoformat()} "
                            f"option={current_option_sym or current_option_key}"
                        )
                    except Exception:
                        pass
                    if not broker:
                        _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] broker not resolved; cannot EXIT on cutoff")
                    else:
                        ex = _exit_sell(
                            user_id=user_id,
                            broker=broker,
                        ...
                        _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] EXIT (cutoff) res=", ex)
                        if ex.get("ok"):
                            in_trade = False
                            position_side = None
            ...
            if now >= next_ohlc_at:
                ...
                if last_processed_candle_ts == ts_now:
                    try:
                        _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] skipping duplicate candle ts={ts_now}")
                    except Exception:
                        pass
                    time.sleep(IDLE_SLEEP_SEC)
                    continue
                ...
                try:
                    _log(
                        f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT][OPTION_OHLC] "
                    ...
                    _log(
                        f"[NIFTY_ATM_WMA5_SMA1_OPEN_OFFSETM1_PUT][CLOSE] ts={ts_now} "
                ...
                # Signal: strict crossover using prev + current candles with offset -1 on both indicators
                if (
                ...
                try:
                    _log(
                        f"[NIFTY_ATM_WMA5_SMA1_OPEN_OFFSETM1_PUT][CROSSCHK] ts={ts_now} "
                    ...
                    if position_side == "LONG" and bearish_cross:
                        try:
                            ...
                            if last_exit_ts is not None and last_exit_ts == ts_now:
                                _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] skipping duplicate exit ts={ts_now} because last_exit_ts={last_exit_ts}")
                                time.sleep(IDLE_SLEEP_SEC)
                                continue
                        except Exception:
                            pass
                        try:
                            _log(
                                f"[NIFTY_ATM_WMA5_SMA1_OPEN_OFFSETM1_PUT][EXIT] bearish_cross ts={ts_now} "
                                f"prev(wma5_ha_close_offset_m1={wma_close_prev} sma1_ha_open_offset_m1={sma_open_prev}) "
                                f"now(wma5_ha_close_offset_m1={wma_close_now} sma1_ha_open_offset_m1={sma_open_now})"
                            )
                        except Exception:
                            pass
                        if not broker:
                            _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] broker not resolved; cannot EXIT long")
                        else:
                            ex = _exit_sell(
                                user_id=user_id,
                                broker=broker,
                            ...
                            _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] EXIT LONG res=", ex)
                            if ex.get("ok"):
                                in_trade = False
                                position_side = None
                                current_option_key = None
                                ...
                                try:
                                    ...
                                    _write_persisted_last_exit_ts(user_id, last_exit_ts)
                                    _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] last_exit_ts set={last_exit_ts} after EXIT LONG (persisted)")
                                    ...
                                    try:
                                        ...
                                        _log(
                                            f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT][FILL][EXIT] candle_ts={ts_now} wall_ts={wall_ts} "
                ...
                if bullish_cross:
                    ...
                    try:
                        ...
                        if last_exit_ts is not None and last_exit_ts == ts_now:
                            _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] skipping bullish entry ts={ts_now} because last_exit_ts={last_exit_ts}")
                            time.sleep(IDLE_SLEEP_SEC)
                            continue
                        ...
                        if last_entry_ts is not None and last_entry_ts == ts_now:
                            _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] skipping duplicate bullish entry ts={ts_now} because last_entry_ts={last_entry_ts}")
                            time.sleep(IDLE_SLEEP_SEC)
                            continue
                        pending_claim_ts = _read_pending_entry_claim_ts(user_id)
                        if pending_claim_ts is not None and pending_claim_ts == ts_now:
                            _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] skipping bullish entry ts={ts_now} because pending_entry_claim_ts={pending_claim_ts}")
                            time.sleep(IDLE_SLEEP_SEC)
                            continue
                    except Exception:
                        pass
                    if now >= entry_cutoff_dt:
                        _log(
                            f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] entry cutoff reached {entry_cutoff_dt.strftime('%H:%M')} IST; skipping entry."
                        )
                        time.sleep(IDLE_SLEEP_SEC)
                        continue
                    try:
                    ...
                    if not _claim_entry_candle(user_id, ts_now):
                        _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] entry claim rejected for ts={ts_now}; another runner likely owns it")
                        time.sleep(IDLE_SLEEP_SEC)
                        continue
                    _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] attempting ENTRY LONG ts={ts_now} last_exit_ts={last_exit_ts}")
                    _log(
                        f"[NIFTY_ATM_WMA5_SMA1_OPEN_OFFSETM1_PUT][ENTRY] bullish_cross ts={ts_now} "
                        f"prev(wma5_ha_close_offset_m1={wma_close_prev} sma1_ha_open_offset_m1={sma_open_prev}) "
                        f"now(wma5_ha_close_offset_m1={wma_close_now} sma1_ha_open_offset_m1={sma_open_now}) "
                        f"option={opt_sym or opt_key}"
                    )
                    if not broker:
                        _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] broker not resolved; cannot ENTER long")
                        ...
                        _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] ENTRY LONG res=", ent)
                        if ent.get("ok") and ent.get("option_key"):
                            ...
                            try:
                                ...
                                wall_ts = _now_ist().isoformat()
                                _log(
                                    f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT][FILL][ENTRY] candle_ts={ts_now} wall_ts={wall_ts} "
                                    f"option={current_option_sym or current_option_key} fill_price={fill_price} "
                                    f"order_id={order_id} snapshot_ltp={snapshot_ltp} raw={str(raw)[:400]}"
                                )
                            except Exception:
            ...
            if in_trade and current_option_key:
                ...
                if ltp_opt is not None and compute_strategy_mtm_for_instrument:
                    try:
                        try:
                            _log(
                                f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT][MTM] ts={_now_ist_str()} "
                ...
                if ltp_opt is not None and compute_strategy_mtm_for_today:
                    try:
                        ...
                        if hit_profit or hit_loss:
                            ...
                            _log(
                                f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] AUTO-EXIT {reason.upper()} "
                                f"mode={trade_mode} mtm={mtm_check:.2f} maxP={max_profit:.2f} maxL={max_loss:.2f}"
                            )

                            if current_option_key and current_option_sym:
                                if not broker:
                                    _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] broker not resolved; cannot AUTO-EXIT order")
                                else:
                                    ex = _exit_sell(
                                        user_id=user_id,
                                        broker=broker,
                                    ...
                                    _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] AUTO-EXIT res=", ex)

                            _mark_strategy_stopped(user_id, reason=f"auto_{reason}")
                            _log("[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] Strategy STOPPED after auto-exit. Exiting stream_forever.")
                            break
            ...
            try:
                _log(f"[NIFTY_ATM_WMA5_SMA2_OPEN_PUT] loop error: {e}")
            except Exception:
                pass
            time.sleep(1.0)
